...
#
#HAR_final <- split(HAR_dataset2, list(HAR_dataset2$subject, HAR_dataset2$classlabel))
HAR_final <- split(HAR_dataset2, HAR_dataset2$subject)
# aaa <- lapply(HAR_final, function(x) colMeans(x[,mean_sd_var]))
# 
# df <- data.frame(matrix(unlist(aaa), nrow=180, byrow=T),stringsAsFactors=FALSE)
#
HAR_dataset3 <- c()
#
for (i in 1:30){
        temp <- by(HAR_final[[i]], HAR_final[[i]][,1], function(x) mean(x))
        #apply(HAR_final[[i]],2,mean)
        #temp <- lapply(HAR_final[[i]], mean)
        HAR_dataset3 <- rbind(HAR_dataset3,temp)
        # cleat the "temp" variable!
        rm(temp)
}
# HAR_dataset3 <- merge(activity_labels,HAR_dataset3)
# HAR_dataset4 <- c()
# HAR_dataset4 <- lapply(HAR_final, mean)
#########################################################################################################################################################
# The run_analysis scrip do:                                                                                                                            #        
# 1 - Merges the training and the test sets to create one data set.                                                                                     #       
# 2 - Extracts only the measurements on the mean and standard deviation for each measurement.                                                           #
